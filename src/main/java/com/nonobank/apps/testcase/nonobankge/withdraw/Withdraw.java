...
;		// 接口---绑卡获取动态码
		String response_getDynByBind = getDynByBindTest.degreecard(bankCardNo, bankCode, 
				idCard, userId, sessionId,
				mobile, realName);
		...
		// 接口---绑卡
		saveBankCardTest.saveBankCard(bankCardNo, idCard, bankCode, token_bankcard,
				mobile, validCode, userId,
		...
		String orderNum=jsonObj_generateKQMobileOrderNoByRecharge.get("data").toString();
		//接口-----快速支付发送验证码-账号余额充值
		String response_getPayGNumByRecharge=getPayGNumByRechargeTest.
				getPayGNumByRecharge(rechargeMoney, newZFPwd, 
				sessionId, userBankCardId, orderNum);
		JSONObject jsonObj_getPayGNumByRecharge=JSON.
				parseObject(response_getPayGNumByRecharge);
		...
		qucikDynPayTest.qucikDynPay(rechargeMoney, token_recharge,
				userBankCardId, bankSmsCode, orderNum, sessionId);
		//点击我的
		biz_common.click_me();		
		//我的----提现
